...
export class QueryList<T> extends React.Component<
  IQueryListProps<T>,
  IQueryListState<T>
> {
  public static displayName = `Aluminum.QueryList`
  ...
  private renderItemList = (listProps: IItemListRendererProps<T>) => {
    ...
    const createItemView = this.isCreateItemRendered()
      ? this.renderCreateItemMenuItem(this.state.query)
      : null
  ...
  private canCreateItems(): boolean {
    return (
      this.props.createNewItemFromQuery != null &&
...
function getFilteredItems<T>(
  query: string,
  { items, itemPredicate, itemListPredicate }: IQueryListProps<T>
) {
