...
<Button
  onClick={() =>
    toaster.notify(
      'ParishKit is a open-source design system'
    )
  }
>
  Notify
...
Passing a unique ID via `id` property allows ParishKit to close all previous toasts with the same ID, before showing a new one.
