...
The `Pane` and `Card` components are one of the most important components in ParishKit.
...
The `Pane` and `Card` components map almost directly to the `Box` from [`aluminum-box`](https://github.com/hennessyevan/aluminum-box).
The `Box` component is a UI Primitive component that is used as the base of many components in ParishKit.
...
Many components within ParishKit use the `Box` component instead of a element such as `div` or `button`.
...
In ParishKit most of the time you don’t deal with CSS classes through the `className` property.
Instead, you write CSS properties directly onto your ParishKit components —
in most cases it will just work.

_If you know what you are doing and do need to use CSS directly, be aware that mixing
ParishKit with regular CSS might give you unexpected results._
...
```jsx
// import { Pane, Text } from '@hennessyevan/aluminum-ui'
...
## Override single properties when using ParishKit components

Most components in ParishKit use a `Pane` or `Box` under the hood.
To add spacing, or layout properties, pass the properties straight to the component:

```jsx
// import { Pane, Button, Text, Heading } from '@hennessyevan/aluminum-ui'
...
In most cases in ParishKit you can use the `height` property on components to change the size
of a component. The text styles and padding will adjust based on the height you pass.

```jsx
// import { Pane, Button, TextInput } from '@hennessyevan/aluminum-ui'

<Pane>
  <TextInput
    width="100%"
    height={48}
    placeholder="Change my height to fit your needs."
  />
  <Button height={48} appearance="primary" marginTop={16}>
    Big Button
  </Button>
</Pane>
```
...
Within ParishKit we call this the major scale.

#### ParishKit exports a utility called `majorScale` to help conform to the 8px major scale.
...
```jsx
// import { majorScale, Button } from '@hennessyevan/aluminum-ui'
...
#### ParishKit exports a utility called `minorScale` to help conform to the 4px minor scale.
...
```jsx
// import { minorScale, Button } from '@hennessyevan/aluminum-ui'
...
Currently there is no opinionated way to construct responsive layouts in ParishKit.
...
In the case when you need to pass properties to a ParishKit component based on the viewport,
...
The `Pane` component maps almost directly to the `Box` from `aluminum-box`.
